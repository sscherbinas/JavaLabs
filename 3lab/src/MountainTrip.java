public class MountainTrip extends Tour {
    private double averageMountainHeight;

    private final TourType tourType = TourType.SPORT;

    public MountainTrip() {

    }

    @Override
    public String toString() {
        return "MountainTrip{" +
                "averageMountainHeight=" + averageMountainHeight +
                ", tourType=" + tourType +
                ", name='" + name + '\'' +
                ", duration=" + duration +
                ", price=" + price +
                ", country='" + country + '\'' +
                ", tourType=" + tourType +
                '}';
    }

    public MountainTrip(String name, int duration, double price, String country, double averageMountainHeight) {

        super(name, duration, price, country);
        this.averageMountainHeight = averageMountainHeight;
    }


    public double getAverageMountainHeight() {

        return averageMountainHeight;
    }

    public void setAverageMountainHeight(double averageMountainHeight) {

        this.averageMountainHeight = averageMountainHeight;
    }

    public TourType getTourType() {
        return tourType;
    }
}
